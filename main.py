...
def page_achievement_ratio():
    ...
    with st.form(key=f"{selected_subject}_{selected_sub_subject}"):
        ...
        if st.form_submit_button("저장"):
            total_ratio = ratio_A + ratio_B + ratio_C + ratio_D + ratio_E
            if total_ratio != 1.0:
                st.error("성취도 비율의 합이 1이 되어야 합니다.")
            else:
                achievement_ratio_data = {
                    "A": ratio_A,
                    "B": ratio_B,
                    "C": ratio_C,
                    "D": ratio_D,
...
def main():
    ...
    pages = {
        ...
        "과목별 준비 사항 및 특징 입력": page_subject_preparation  # 새로운 페이지 추가
    }
